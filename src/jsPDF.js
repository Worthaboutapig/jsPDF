 ...
 * @constructor
 */
export default function jsPDF(options) {
  ...
  API.__private__.rect = API.rect = function (x, y, w, h, style, patternKey, patternData) {
    ...
    const _x = hpf(scale(x));
    const _y = hpf(transformScaleY(y));
    const _w = hpf(scale(w));
    const _h = hpf(scale(h));
    const str = [_x, _y, _w, _h, "re"].join(" ");
    out(str);
...
jsPDF.addModule = function (module, libraries) {
  if (!(typeof module == "function")) {
    throw new Error(`Module ${module} not loaded.`);
  }

  module(jsPDF.API, libraries);
};
